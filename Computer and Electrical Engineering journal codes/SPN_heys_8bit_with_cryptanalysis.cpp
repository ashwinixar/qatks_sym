...
Both Linear and Differential Cryptanalysis are perfomed
Linear aprroximation relation: (0x10,0x0E)
Differential relation: (0x40,0x0A)
Linear cryptanalysis produces last-round's subkey's rightmost 4-bit correctly with highest bias of 0.210938
Differential cryptanalysis produces last-round's subkey's rightmost 4-bit correctly with highest bias of 0.0234375
...
void lin_last_rounds_attack()
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		cout << (((db / 2.0) - subkeys[i]) / (float)db) << endl;
	}
}

//ENDS - Linear Cryptanalysis

//STARTS - Differential Cryptanalysis
...
void diff_last_rounds_attack()
{
	...
	for (int i = 0; i < 16; i++) //Loop for each subkey
	{
		for (int j = 0; j < db; j += 2) //Loop for each data (ciphertext) last round backward operation
		{
			block c1 = C[j] ^ i, c2 = C[j + 1] ^ i;
			s_box_inv(c1);
			s_box_inv(c2);
			if ((c1 ^ c2) == 0x0A)
				subkeys[i]++;
	...
	{
		cout << hex << uppercase << (unsigned int)i << ": ";
		cout << ((double)subkeys[i] / (db / 2.0)) << endl;
	}
}

//ENDS - Differential Cryptanalysis
...
int main()
{
	...
	/*
	block p = 0x87, c = 0;
	cout << hex << uppercase << (unsigned int)p << endl;
	c = oracle_enc(p);
	cout << hex << uppercase << (unsigned int)c << endl;
	p = oracle_dec(c);
	cout << hex << uppercase << (unsigned int)p << endl;
	*/
	cout << "Linear Cryptanalysis:\n";
	lin_last_rounds_attack();
	cout << endl << "Differential Cryptanalysis:\n";
